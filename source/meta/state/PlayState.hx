...
class PlayState extends MusicBeatState
{
	...
	{
		dialogueLangSuffix = (ClientPrefs.data.language != 'en') ? '-' + Localization.currentLanguage : '';
		...
		comboFunction = () -> {
			// Rating FC
			switch (ClientPrefs.data.scoreTxtType)
			{
				case 'Default':
					ratingFC = "CB";
					if (songMisses < 1){
		...
		switch (curStage)
		{
			...
			case 'tank': //Week 7 - Ugh, Guns, Stress
				...
				if(!ClientPrefs.data.lowQuality) foregroundSprites.add(new BGSprite('$path/tank1', -300, 750, 2, 0.2, ['fg']));
				foregroundSprites.add(new BGSprite('$path/tank2', 450, 940, 1.5, 1.5, ['foreground']));
				if(!ClientPrefs.data.lowQuality) foregroundSprites.add(new BGSprite('$path/tank4', 1250, 900, 1.5, 1.5, ['fg']));
				foregroundSprites.add(new BGSprite('$path/tank5', 1620, 700, 1.5, 1.5, ['fg']));
				if(!ClientPrefs.data.lowQuality) foregroundSprites.add(new BGSprite('$path/tank3', 1300, 1200, 3.5, 2.5, ['fg']));
		}
		...
		healthBarBG = new AttachedSprite((ClientPrefs.data.longBar) ? 'healthBarLong' : 'healthBar');
		...
		if (gf.antialiasing == true)
			gf.antialiasing = ClientPrefs.data.globalAntialiasing;
	...
	{
		var scoreThing = (ClientPrefs.data.weekendScore) ? truncateFloat(smoothScore, 0) : songScore;
		switch (ClientPrefs.data.scoreTxtType)
		{
			case 'Default':
				if(ratingName == '?') {
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1)
				targetAlpha = (!ClientPrefs.data.opponentStrums) ? 0 : (ClientPrefs.data.middleScroll) ? 0.35 : 1;
	...
	public function endSong():Void
	{
		...
		if (!inReplay)
		{
			...
			if (ClientPrefs.data.saveReplay)
	...
	private function popUpScore(?note:Note, ?optionalRating:Float):Void
	{
		...
		var precision:FlxText = new FlxText(0, (ClientPrefs.data.downScroll ? playerStrums.members[0].y + 110 : playerStrums.members[0].y - 40), '' + Math.round(Conductor.songPosition - note.strumTime) + ' ms');
		precision.cameras = [camOther];
		if (ClientPrefs.data.downScroll) precision.y -= 3; else precision.y += 3;
		...
		for (i in seperatedScore)
		{
			...
			if (ClientPrefs.data.displayMilliseconds) add(precision);
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED)))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				if (sortedNotesList.length > 0) {
					...
					if (canMiss && !ClientPrefs.data.ghostTapping) {
						noteMissPress(key);
					}
				}
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		{
			col = ClientPrefs.data.arrowRGB[data][0];
			if(note != null) {
